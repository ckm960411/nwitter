...
function Home({ userObj }) {
  ...
  const onSubmit = async (event) => {
    ...
    if (attachment === "" && fileInputRef === false) return;
    let attachmentURL = "";
    if (attachment !== "") {
      // 파일 경로 참조 만들기
  ...
  const onFileChange = (event) => {
    ...
    reader.onloadend = (finishedEvent) => setAttachment(finishedEvent.currentTarget.result);
  ...
  return (
    <div>
      <form onSubmit={onSubmit}>
        ...
        {attachment && (
          <div>
            <img src={attachment} width="50px" height="50px" alt="myUploadedImg" />
